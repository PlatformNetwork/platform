...
For the `3.0.1` release, the Python package version, Helm chart `version`, and Helm `appVersion` are all `3.0.1`. The Git release tag is `v3.0.1`.
...
Use Git tags with a leading `v`, such as `v3.0.1`, for release events. The GitHub Actions metadata policy publishes canonical GHCR image tags from the tag event using:
...
This means a `v3.0.1` Git tag publishes both the canonical `3.0.1` image tag and the compatibility `v3.0.1` tag, plus a traceable `sha-<commit>` tag. The `latest` tag is published only from `main` and is a mutable development/staging channel.
...
```text
ghcr.io/platformnetwork/platform:3.0.1@sha256:<64-hex-digest>
```
